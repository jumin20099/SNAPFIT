...
public interface CommentRepository extends JpaRepository<Comment, Long> {
    ...
    @Query("SELECT c.post.postId as postId, COUNT(c) as commentCount " +
           ...
           "ORDER BY " +
           "  CASE :sortBy " +
           "    WHEN 'postId' THEN postId " +
           "    WHEN 'commentCount' THEN commentCount " +
           "    ELSE commentCount " +
           "  END " +
           "  CASE :sortOrder " +
           "    WHEN 'ASC' THEN ASC " +
           ...
           "    ELSE DESC " +
           "  END")
    List<Object[]> getCommentTrendByPostAndDateRangeAndSearchTermAndSort(@Param("startDate") LocalDateTime startDate, 
                                                                        @Param("endDate") LocalDateTime endDate, 
                                                                        @Param("searchTerm") String searchTerm, 
    ...
    @Query("SELECT c.author.userIdx as authorId, COUNT(c) as commentCount " +
           ...
           "    ELSE DESC " +
           "  END")
    List<Object[]> getCommentTrendByAuthorAndDateRangeAndSearchTermAndSort(@Param("startDate") LocalDateTime startDate, 
                                                                          @Param("endDate") LocalDateTime endDate, 
                                                                          @Param("searchTerm") String searchTerm, 
                                                                          @Param("sortBy") String sortBy, 
                                                                          @Param("sortOrder") String sortOrder);
    ...
    /**
     * 댓글 트렌드 조회 (게시글별 + 날짜 범위 + 검색어 + 정렬 + 페이징)
     * 성능: 복합 조건 집계 + 검색 + 정렬 + 페이징
     */
    @Query("SELECT c.post.postId as postId, COUNT(c) as commentCount " +
           "FROM Comment c " +
           "WHERE c.createdAt BETWEEN :startDate AND :endDate " +
           ...
           "    ELSE DESC " +
           "  END")
    Page<Object[]> getCommentTrendByPostAndDateRangeAndSearchTermAndSort(@Param("startDate") LocalDateTime startDate, 
                                                                        @Param("endDate") LocalDateTime endDate, 
                                                                        @Param("searchTerm") String searchTerm, 
                                                                        @Param("sortBy") String sortBy, 
                                                                        @Param("sortOrder") String sortOrder, 
    ...
    /**
     * 댓글 트렌드 조회 (사용자별 + 날짜 범위 + 검색어 + 정렬 + 페이징)
     * 성능: 복합 조건 집계 + 검색 + 정렬 + 페이징
     */
    @Query("SELECT c.author.userIdx as authorId, COUNT(c) as commentCount " +
           "FROM Comment c " +
           ...
           "    ELSE DESC " +
           "  END")
    Page<Object[]> getCommentTrendByAuthorAndDateRangeAndSearchTermAndSort(@Param("startDate") LocalDateTime startDate, 
                                                                          @Param("endDate") LocalDateTime endDate, 
                                                                          @Param("searchTerm") String searchTerm, 
                                                                          @Param("sortBy") String sortBy, 
                                                                          @Param("sortOrder") String sortOrder, 
